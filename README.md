 ...
 * install gunicorn by running <python3.6 -m  pip install gunicorn>
 * This is a unix based server that will run our application on Heroku

 <!-- + Create a heroku account to act as hosting environment... -->
 + install the heroku CLI <sudo snap install --classic heroku>(ubuntu)

 + pip freeze to list all dependencies required <pip freeze>

 + Install all requirements needed bt running...
    run<pip freeze > requirements.txt>

 + Create a Procfile
  run <touch Profile>

 + Add <web: gunicorn manage:app
>  to the procfile you created

 + Create heroku application
   * <heroku login> after this run <heroku create<name of application>>
   <!-- nb/--for this you can ignore the apllication name and heroku will create one for you but if you have a unique name the dont hesitate use it. -->

+ Adding Configurations to heroku...
  * run heroku config:set MOVIE_API_KEY=<YOUR MOVIE API> if you used an API
  * heroku config:set SECRET_KEY=<YOUR SECRET KEY> if you had set a secret key....

  + Deploy 
   * run <git add .>
   * run <git commit -m 'deployment to hrku'>
   * run <git push heroku master >

NB/ Heroku goes through your requirements.txt file,installs all our dependencies and builds our application
 /..incase you get any error after all these commands try run <heroku logs --tail>
...
Email: developersjuniors@gmail.com
Password (typing will be hidden):
Authentication successful.
```

# Preparing the Application
In this tutorial I will deploy an existing project, [moringa-tribune-hosting](https://github.com/hussein18149/mtribune-hosting).
It's an very simple open-source Django project, that shows news posted.
Its available on [github](https://github.com/hussein18149/mtribune-hosting) so you can actually clone the repository and follow along or try it on your own existing django project.
